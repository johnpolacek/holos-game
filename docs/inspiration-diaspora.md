...
and traffic. No dial moves; nothing is scored. It is one stored
...
reunion payload* and *The lineage referendum* — with a pointer from
§ Contact's signal payloads and from § Coalitions; thin with signals
...
a no-sleep frame): traffic as the fifth drift pressure — an
...
threshold. Signals already carry culture as dial nudges — *contact
...
- **The reunion payload** *(working label)*. A signal class that only
...
to be a wire-visible signal class, which is more than a derived
quantity. The thin version — the payload as fiction over the existing
knowledge/culture signal effects, family-flavored in prose only — costs
nothing and proves the appetite. Lands with signals (A2–A3) thin, and
...
ground, and the grief of the unbound is that no signal will ever land
...
and **reachability** — traffic and beats that argue with the
bound value no longer land, rendered not as refusal but as the mind's
serene non-answer, which reads to a counterpart exactly as the book
...
costs nothing. The register: a slow-running civilization's signals
should read as if the light-lag were conversational — because for it,
it is. A traffic beat is allowed to say so: *their reply took
...
counterpart thinks ten signals between yours.
...
- **The rescue** *(a beat class)*. The offer the burst makes possible:
  ...
  what an anomaly's traffic surge is *about*.
...
| 3 | The merge + family referendum | Payload stack, culture nudges, Ledger, mission-clock derivations | **Adopted (2026-07), renamed** — act3-design.md § Charters and the Ledger (reunion payload, lineage referendum) | Medium; wire question flagged |
...
- **Does the reunion payload need a wire concept?** The thin version
  (family-flavored prose over existing signal effects) does not; the
  full version (a first-class signal kind with integrate/decline) might.
