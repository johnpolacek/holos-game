...
| Session zero — inheritance | *(generate next)* | **Wanted now — A1 #1** | The new game entry (Act 3 first): choose + name a generated civilization. Prompt written — ui-image-brief.md screen 16. No player sees anything before this, so it's the first image to make. Suggested file: `03-00-inheritance.png`. |
