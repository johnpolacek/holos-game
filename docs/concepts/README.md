...
| Session zero — inheritance | *(regen wanted)* | **Layout adopted; relabel** | First render nailed the layout, the succession feeling, and the charters (the strongest emotional hit of the set — it beats the inherited-vs-owned risk). One fix: it invented dial labels. The in-world labels are now pinned (act2-design.md § In-world labels): **Reach·Depth, Voice·Silence, Garden·Forge, Monolith·Chorus, Memory·Renewal**; archetypes stay canonical (Herald ✓; the render's Keeper/Gardener → Monument/Shepherd). Regen with the updated screen-16 prompt, then commit as `03-00-inheritance.png`. |
| Act 3 — Sky + source card | *(generate next)* | **Wanted now — A1** | ui-image-brief.md screen 7. The observatory / classification screen — A1's second surface. |
