...
export class Cohort extends Server<CohortEnv> {
  ...
  private sourceChangePoints(
    ...
    // OLDEST-FIRST at the cap, so a later scan finds the same early points.
  ...
  private findFires(
    ...
    for (const source of visibleSky(galaxy, civId, toYear)) {
      ...
      // points. `prev` starts at the window's own floor: the board as it stood
      // when this walk began looking. A FIRST APPEARANCE THEREFORE NEVER
      // FIRES, which is right: a source whose first arrived light already
      // favors one reading has not crossed anything, it has only shown up.
